...
*/
	...
	ScanMatcher::ScanMatcher(double minUrange) : m_laserPose(0, 0, 0)
	{
		//m_laserAngles=0;
		m_laserBeams = 0;
		m_optRecursiveIterations = 3;
...
*/
	void ScanMatcher::computeActiveArea(ScanMatcherMap &map, const OrientedPoint &p, const double *readings)
	{
		...
		for (const double *r = readings + m_initialBeamsSkip; r < readings + m_laserBeams; r++, angle++)
			...
			{
				double d = *r;
				if (d > m_laserMaxRange || d == 0.0 || isnan(d) || d < m_usableMinRange)
					continue;
				if (d > m_usableRange)
					d = m_usableRange;


				IntPoint p0 = map.world2map(lp + Point(m_usableMinRange * cos(lp.theta + *angle), m_usableMinRange * sin(lp.theta + *angle)));//The closest visible point is at least at distance m_usableMinRange
			...
			else
			{
				...
				if (*r > m_laserMaxRange || *r > m_usableRange || *r == 0.0 || isnan(*r) || (*r < m_usableMinRange))
					continue;
				Point phit = lp;
				phit.x += *r * cos(lp.theta + *angle);
				phit.y += *r * sin(lp.theta + *angle);
...
*/
	...
	double ScanMatcher::registerScan(ScanMatcherMap &map, const OrientedPoint &p, const double *readings)
	{
		...
		//m_usableMinRange = 1.5;
		//std::cout << "m_usableMinRange:" << m_usableMinRange << std::endl;
...
*/
	...
	*/
        //std::cout << " beams:" << beams << " LASER_MAXBEAMS:" << LASER_MAXBEAMS << std::endl;
	...
	//Here I have defined a new parameter that doesn't exist in the official version: min_range. This sets the minimum usable sensor range>
	//This is to don't consider the area around the sensor which usually corresponds to the robot's body
	void ScanMatcher::setMatchingParameters(double min_urange, double urange, double range, double sigma, int kernsize, double lopt, double aopt, int iterations, double likelihoodSigma, unsigned int likelihoodSkip)
	{
		m_usableMinRange = min_urange; //Set the minimun range
