...
@section services
 ...
 - @b "~dynamic_map_metadata" : returns the map metadata
...
void SlamGMapping::startLiveSlam()
{
  ...
  ssm_ = node_.advertiseService("dynamic_map_metadata", &SlamGMapping::mapMetaDataCallback, this);
...
bool
SlamGMapping::mapMetaDataCallback(gmapping::GetMapMetaData::Request  &req,
                                  gmapping::GetMapMetaData::Response &res)
{
  boost::mutex::scoped_lock map_lock (map_mutex_);
  if(got_map_ && map_.map.info.width && map_.map.info.height)
  {
    res.metadata = map_.map.info;
    return true;
  }
  else
    return false;
